...
/**
 * Clean up duplicate patrol visits — the same site, exact time and kind
 * appearing more than once (the artefact of the pre-fix schedule-churn bug).
 * Keeps one visit per slot and cancels the extras; only ever cancels PENDING
 * visits (never actioned/completed work), preferring to keep an actioned or
 * schedule-linked visit. Admin only. Safe to run repeatedly.
 */
export async function dedupePatrolVisits(): Promise<{
  ok: boolean;
  groups?: number;
  cancelled?: number;
  error?: string;
  ...
  if (toCancel.length === 0) {
    return { ok: true, groups: dupGroups, cancelled: 0 };
  }
  ...
  try {
    for (const id of toCancel) {
      ...
      await logActivity({
        ...
        action: "cancelled",
        userId: me.id,
        diff: { reason: "duplicate patrol removed" } as any,
      });
    }
  } catch (e: any) {
  ...
  revalidatePath("/activities");
  revalidatePath("/finance");
  return { ok: true, groups: dupGroups, cancelled: toCancel.length };
}
